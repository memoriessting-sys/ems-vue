<template>
  <el-card shadow="hover" class="page-card">
    ...
    <div class="toolbar">
      ...
      <el-button type="success" @click="openDialog()" v-permission="'dept:add'" class="add-btn"><el-icon><Plus /></el-icon>新增部门</el-button>
    </div>
    <el-table :data="list" border stripe class="data-table" :header-cell-style="{background:'#f5f7fa',color:'#606266'}">
      ...
      <el-table-column label="操作" width="200" align="center">
        <template #default="{ row }">
          <el-button link type="primary" @click="openDialog(row)" v-permission="'dept:edit'" class="op-btn"><el-icon><Edit /></el-icon>编辑</el-button>
          <el-popconfirm title="确认删除该部门?" @confirm="handleDelete(row.id)">
            <template #reference><el-button link type="danger" v-permission="'dept:delete'" class="op-btn"><el-icon><Delete /></el-icon>删除</el-button></template>
          </el-popconfirm>
        </template>
      </el-table-column>
    </el-table>
